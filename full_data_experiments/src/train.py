...
def train_sweep(seed, train_loader, test_loader, model_name, n_heads, n_layers, lr, wd, num_epochs, loss_mode, K, data, A1, data_summary, mode, wb = False):
  metrics = [[], []]
  seeds_list = [43, 1337, 7, 777, 9876] #, 54321, 123456, 999, 31415, 2022]
  for seed_ in seeds_list:
    accuracy, f1_s = train_generic(seed_, train_loader, test_loader, model_name, n_heads, n_layers, lr, wd, num_epochs, loss_mode, K, data, A1, data_summary, mode, wb = wb)
...
def train_generic(seed, train_loader, test_loader, model_name, n_heads, n_layers, lr, wd, num_epochs, loss_mode, K, data, A1, data_summary, mode, config, wb = False, save_model = False, random_feat = False):
  ...
  if model_name == 'GAT':
    exp_name = f"{model_name}_{seed}_{n_layers}_{n_heads}_{lr}_{wd}_{loss_mode}_{random_feat}"
    model = GATSY(n_heads, n_layers)
    predictor = None

  elif model_name == 'SAGE':
    exp_name = f"{model_name}_{seed}_{n_layers}_{lr}_{wd}_{loss_mode}_{random_feat}"
    model = GraphSage()
    predictor = None
  
  elif model_name == 'GCN':
    exp_name = f"{model_name}_{seed}_{n_layers}_{lr}_{wd}_{loss_mode}_{random_feat}"
    model = GCNSY(n_layers = n_layers)
    predictor = None
  
  elif model_name == 'GIN':
    exp_name = f"{model_name}_{seed}_{n_layers}_{lr}_{wd}_{loss_mode}_{random_feat}"
    model = GINSY(n_layers = n_layers)
    predictor = None

  elif model_name == 'GRAFF':
    exp_name = f"{model_name}_{seed}_{n_layers}_{lr}_{wd}_{loss_mode}_{random_feat}"
    model = GRAFF(num_layers = n_layers)
    predictor = None
  ...
  early_stop = EarlyStopping(monitor='Loss on test (UNSPV)', patience=30, mode="min")
  ...
  if wb:
      ...
      trainer = pl.Trainer(
          ...
          accelerator = 'gpu', devices = num_gpu, # the number of gpus we have at our disposal.
  ...
  if save_model:
    torch.save(test_model.model, f"./Models/{exp_name}.pt")
